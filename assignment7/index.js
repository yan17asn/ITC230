...
app.get('/', (req, res)=>{
    ...
    Book.find({}, (err, items) => {
       if (err) return next(err);
        res.type('text/html');
       res.render('home',{result:JSON.stringify(items)});
    });
});
...
app.post('/api/v1/book/add',(req,res)=>{
    ...
    bookrecord.save((err,result)=>{
        let added = result.n !== 0;
                    if (added){
             Book.count((err,total)=>{
                res.json({_id:result._id, title:result.title, result:'added',updated: 0, remain:total});
           });     
       }      
        else
        ...
        Book.updateOne({title:req.body.title}, {author: req.body.author, price: req.body.price,inventory:req.body.inventory }, (err, result) => {
            if (err) return next(err);
            res.json({updated: result.nModified, _id:result._id,title:req.body.title});
        });
    }
    })
});
